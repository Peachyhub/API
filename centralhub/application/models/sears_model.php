...
class Sears_model extends CI_Model {
    ...
    public function saveProduct($data = array()) {
        if (!empty($data['part_number'])) {
            ...
            $this->db->select('product_id,category_id,part_number,cat_entry_id,display_price,stock_indicator,cut_price');
	    ...
	    if(isset($data['mfg_part_number'])){
            ...
            if ($query->num_rows()) {
                ...
                $result = $query->row();   
		if($result->display_price!=trim($data['display_price']) || $result->stock_indicator!=trim($data['stock_indicator']) || $result->cut_price!=trim($data['cut_price'])) {
    ...
    public function checkProductStatusFromAPI($partNumber) {
        $this->db->select('retailer_name,part_number');
        $this->db->from('tbl_product_details');
